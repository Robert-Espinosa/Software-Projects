...
public final class ArraysAndReferences {
    ...
    private static NaturalNumber productOfArrayElements(
        ...
        // TODO - fill in body
        NaturalNumber sum = new NaturalNumber2(1);
    ...
    private static void computePartialProducts(NaturalNumber[] nnArray) {
        ...
        // TODO - fill in body
        NaturalNumber[] copy = new NaturalNumber1L[nnArray.length];

        for (int i = 0; i < nnArray.length; i++) {
            copy[i] = new NaturalNumber1L(nnArray[i]);
        }

        for (int j = 0; j < nnArray.length; j++) {
            NaturalNumber[] NumsToJ = new NaturalNumber1L[j + 1];

            for (int q = 0; q < NumsToJ.length; q++) {
                NumsToJ[q] = new NaturalNumber1L(copy[q]);
            }

            nnArray[j].copyFrom(productOfArrayElements(NumsToJ));

        }
        for (NaturalNumber q : nnArray) {
    ...
    private static NaturalNumber[] partialProducts(NaturalNumber[] nnArray) {
        ...
        NaturalNumber[] copy = new NaturalNumber1L[nnArray.length];
        NaturalNumber[] finals = new NaturalNumber1L[nnArray.length];

        for (int i = 0; i < nnArray.length; i++) {
            copy[i] = new NaturalNumber1L(nnArray[i]);
        }
        for (int i = 0; i < nnArray.length; i++) {
            finals[i] = new NaturalNumber1L(0);
        }
...
//        computePartialProducts(array);
        ...
        NaturalNumber[] partial = partialProducts(array);
        for (int i = 0; i < partial.length; i++) {
            out.println(partial[i]);
        }
