...
public final class CryptoUtilities {
    ...
    public static boolean isEven(NaturalNumber n) {
        //makes a boolean value for even as returns weather last digit
        //is divisable by two
        boolean even = false;
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
         ...
         */
        // Initialize natural numbers to be used later
        NaturalNumber one = new NaturalNumber2(1);
        NaturalNumber two = new NaturalNumber2(2);
        NaturalNumber pSecond = new NaturalNumber2(p);
        NaturalNumber nSecond = new NaturalNumber2(n);

        if (pSecond.isZero()) {
            //any number to the power of zero is 1
            n.transferFrom(one);
        } else if (isEven(pSecond)) {

            pSecond.divide(two);
            powerMod(n, pSecond, m);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        // makes varibles to be used later
        NaturalNumber one = new NaturalNumber2(1);
        NaturalNumber two = new NaturalNumber2(1);
        NaturalNumber copyW = new NaturalNumber2(w);
        NaturalNumber copyN = new NaturalNumber2(n);
        NaturalNumber copyWtwo = new NaturalNumber2(w);

        //sets copyW to be equal to power 2 mod n
        powerMod(copyW, two, n);
        copyN.decrement();
        powerMod(copyWtwo, copyN, n);

        return copyW.equals(one) || !copyWtwo.equals(one);
    ...
    public static boolean isPrime2(NaturalNumber n) {
         ...
         */
        final int intfour = 4;
        final int testNumber = 50;

        NaturalNumber four = new NaturalNumber2(intfour);
        ...
        if (n.compareTo(four) < 0) {
            //all vals below 4 are prime
            isPrime = true;
        } else if (isEven(n)) {
            //all even vals arent prime
            isPrime = false;
        } else {

            //loop through number of test cases
            while (count < testNumber) {
                NaturalNumber guessVal = randomNumber(nCopy);

                // keep reseting guessVal until it fits requirements
                while (guessVal.compareTo(one) <= 0
                        || guessVal.compareTo(nCopy) >= 0) {
                    guessVal = randomNumber(nCopy);
                }

                if (isWitnessToCompositeness(guessVal, n)) {
                    isPrime = false;
                }
                count++;
            }
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        NaturalNumber one = new NaturalNumber2(1);
        NaturalNumber two = new NaturalNumber2(2);
        NaturalNumber three = new NaturalNumber2(intThree);

        if (n.compareTo(three) >= 0) {
            //make n odd if it is even because all prime numbers are odd
            if (isEven(n)) {
                n.add(one);
            }
            //while not likely prime add two and keep checking odd numbers
            while (!isPrime2(n)) {
                n.add(two);
            }
        }
