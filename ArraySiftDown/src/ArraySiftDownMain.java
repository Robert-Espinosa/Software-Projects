...
public final class ArraySiftDownMain {
    ...
    private static void siftDown(int[] array, int top, int last) {
        ...
        if (rightSubTree <= last) {
            int root = array[top];
            if (array[leftSubTree] < array[rightSubTree]) {

                if (root > array[leftSubTree]) {
                    exchangeEntries(array, leftSubTree, top);
                    siftDown(array, leftSubTree, last);
                }
            } else if (array[leftSubTree] > array[rightSubTree]) {
                if (root > array[rightSubTree]) {
                    exchangeEntries(array, rightSubTree, top);

                    siftDown(array, rightSubTree, last);
                }
            }
