...
public final class RSSReader {
    ...
    private static void processItem(XMLTree item, SimpleWriter out) {
         ...
         * Doesn't return -1 or outputs the description
         */
        if (titleLocation != -1) {
            /*
            ...
            if (item.child(titleLocation).numberOfChildren() > 0) {
                if (!item.child(titleLocation).label().isBlank()) {
                    title = item.child(titleLocation).child(0).label();
                }
            }
        } else {
            if (!item.child(descriptionLocation).label().isBlank()) {
                title = item.child(descriptionLocation).child(0).label();
            }
        }
