...
public final class ExpressionEvaluator {
    ...
    private static int valueOfDigitSeq(StringBuilder source) {
        ...
        int singleDigit = valueOfDigit(source);
        int digits = singleDigit;
        while (Character.isDigit(source.charAt(0))) {
            singleDigit = valueOfDigit(source);
            digits = (digits * RADIX) + singleDigit;
        }
    ...
    private static int valueOfTerm(StringBuilder source) {
        ...
        int returneVal = valueOfFactor(source);
        while (source.charAt(0) == '*' || source.charAt(0) == '/') {
            Character opperation = source.charAt(0);
            source.deleteCharAt(0);

            if (opperation == '*') {
                returneVal *= valueOfFactor(source);
            } else {
                returneVal /= valueOfFactor(source);
            }
        }
    ...
    public static int valueOfExpr(StringBuilder source) {
        ...
        int returneVal = valueOfTerm(source);
        while (source.charAt(0) == '+' || source.charAt(0) == '-') {
            Character opperation = source.charAt(0);
            source.deleteCharAt(0);
            if (opperation == '+') {

                returneVal += valueOfTerm(source);
            } else {
                returneVal -= valueOfTerm(source);
            }
        }
